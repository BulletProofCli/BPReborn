...
---------------Lock Wout-------------------
local function lock_bkword(msg, data, target)
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
if not is_mod(msg) then
if not lang then
...
local lock_bkword = redis:get(target.."bkword")
 if lock_bkword then
if not lang then
 return "*‌Wout* _Is Already Locked_"
elseif lang then
 return "لیست کلمات اجباری در گروه هم اکنون در گروه هم اکنون ممنوع است"
end
else
redis:set(target.."bkword",true)
if not lang then
 return "*Wout* _Has Been Locked_"
else
 return "لیست کلمات اجباری در گروه در گروه ممنوع شد"
end
end
end

local function unlock_bkword(msg, data, target)
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
 if not is_mod(msg) then
if not lang then
...
local lock_bkword = redis:get(target.."bkword")
 if lock_bkword then
 redis:del(target.."bkword")
if not lang then
return "*Wout* _Has Been Unlocked_" 
elseif lang then
return "لیست کلمات اجباری در گروه در گروه آزاد شد"
end
else 
if not lang then
return "*Wout* _Is Already Unlocked_"
else
return "لیست کلمات اجباری در گروه هم اکنون در گروه هم اکنون آزاد است"
end
end
end
...
local wout = redis:get(target.."bkword")
if wout then
	wout = 'yes'
else
	wout = 'no'
...
local target = msg.to.id
if not lang then
if matches[2] == "کلمات اجباری" or matches[2] == 'wout'  then
return lock_bkword(msg, data, target)
end
...
if not lang then
if matches[2] == "کلمات اجباری" or matches[2] == 'wout'  then
return unlock_bkword(msg, data, target)
end
if matches[2] == 'لینک' or matches[2] == 'link' then
...
tdcli.changeChatTitle(chat, gp_name, dl_cb, nil)
 ...
 if not data[tostring(chat)]['about'] then
  ...
  if matches[1] == "addwout" and is_mod then
	if redis:sismember(msg.to.id..":blackword", matches[2]) then
		if not lang then
    return "*Word* _Already is in WoutList_"
    else
     return "عبارت مورد نظر وجود دارد"
      end
	else
		redis:sadd(msg.to.id..":blackword", matches[2])
		if not lang then
    return "'_"..matches[2].."_'  *Added To WoutList*"
    else
     return "عبارت  '_"..matches[2].."_'  به لیست کلمات اجباری اضافه شد."
      end
	end
  end
  if matches[1] == "remwout" and is_mod then
	if redis:sismember(msg.to.id..":blackword", matches[2]) then
		redis:srem(msg.to.id..":blackword", matches[2])
		
		return "عبارت مورد نظر از لیست کلمات اجباری حذف شد"
	else
		return "عبارت مورد نظر در لیست کلمات اجباری وجود ندارد"
  end
  end
  if matches[1] == "woutlist" then
	local text = ":لیست کلمات اجباری\n\n"
	local list = redis:smembers(msg.to.id..":blackword")
	for k,v in pairs(list) do
		text = tostring(text) .. "_l" .. tostring(k) .. "l_  " .. check_markdown(v) .. "\n"
	end
	return text
 end
 if matches[1] == "clean" and matches[2] == "woutlist" then
	local list = redis:smembers(msg.to.id..":blackword")
	for k,v in pairs(list) do
		redis:srem(msg.to.id..":blackword",v)
	end
	return "لیست کلمات اجباری پاکسازی شد."
 end  
...
'^(زبان انگلیسی)$',
'^(زبان) (فارسی)$',
"^(پاک کردن) (لیست کلمات اجباری)$",
"^(لیست کلمات اجباری)$",
"^(اضافه به کلمات اجباری) (.*)$",
"^(حذف از کلمات اجباری) (.*)$",
},
patterns_fa = {
command .. "([Ii]d)$",
