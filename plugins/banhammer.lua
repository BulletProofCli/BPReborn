...
if not is_gbanned(data.id_) then
  ...
  if cmd == "delall" then
   ...
   if not lang then
  return tdcli.sendMessage(arg.chat_id, "", 0, "_You can't delete messages_ *bot admins*", 0, "md")
   elseif lang then
  return tdcli.sendMessage(arg.chat_id, "", 0, "*شما نمیتوانید پیام های ادمین های ربات رو پاک کنید*", 0, "md")
   end
  else
...
return tdcli.sendMessage(msg.to.id, msg.id, 0, "*کاربر "..matches[2].." از محرومیت گروه های ربات خارج شد*", 0, "md")
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   ...
   if msg.to.type ~= 'pv' then
 if matches[1]:lower() == "ban" and is_mod(msg) or matches[1] == "بن" and is_mod(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
